...
class Simulator():
    ...
    def simulate(self, impact_model : impact.Model, controller : Controllers.Controller, x0:np.ndarray, xf:np.ndarray, closed_loop:bool=True):
        ...
        if closed_loop:
            # impact_mpc.method(impact.MultipleShooting(N=N_steps, M = 1, intg='rk'))
            # impact_sim = impact_mpc._method.discrete_system(impact_mpc)
            impact_sim = impact_model.sim_step
            X = np.ndarray([len(t), impact_model.nx]) * np.nan
            ...
            for now in range(len(t)-1):
                ...
                temp = impact_sim(x0 = X[now], u = U[now], T = self.dt)
    ...
    def simulate_multi(self, impact_model : impact.Model, controller : Controllers.Controller, x0_combinations:np.ndarray, xf_combinations:np.ndarray, closed_loop:bool=True):
        # Compute all combinations of initial and final conditions
        N_trajectories = x0_combinations.shape[0]*xf_combinations.shape[0]
...
def save_sims(path, X_series:np.ndarray, U_series:np.ndarray, R_series:np.ndarray, T_series:np.ndarray):
    np.savez(path,X_series = X_series,U_series = U_series,R_series = R_series,
        T_series=T_series)
    return print('Simulation data saved in:', path)
...
def plot_io_samples(inputs:np.ndarray, outputs:np.ndarray, colors = ["blue","red","black"], names=["in","out"], bounds=None, max_points=1e5):
    ...
    for i in range(n_in):
        for j in range(n_out):
            fig.add_trace(
                go.Scatter(x=inputs_flat[:, i], y=outputs_flat[:, j], mode='lines', line=dict(color=colors[0]), showlegend=False),
                row=j+1, col=i+1
            )
            fig.add_trace(
            ...
            fig.add_trace(
                go.Scatter(x=inputs[:, -1, i], y=outputs[:, -1, j], mode='markers', line=dict(color=colors[2]), name="Final", showlegend=(i==0 and j==0)),
                row=j+1, col=i+1
            )
            ...
            # set axis labels
            if n_in + n_out == 2:
                fig.update_xaxes(title_text=names[0], row=j+1, col=i+1)
                fig.update_yaxes(title_text=names[1], row=j+1, col=i+1)
            else:
                fig.update_xaxes(title_text=f'{names[0]} {i+1}', row=j+1, col=i+1)
                fig.update_yaxes(title_text=f'{names[1]} {j+1}', row=j+1, col=i+1)
    
    fig.update_layout(height=300*n_out, width=300*n_in, title_text="Inputs vs Outputs Scatter Plots")
    return fig

def plot_time_evolution(X, U, R, t, fig=None, colors = ["blue","red","black"], name="x"):
    nx = X.shape[-1]
    nu = U.shape[-1]
    ...
    for i in range(nx):
        fig.add_trace(go.Scatter(x=t, y=X[:,i], mode='lines', line=dict(color=colors[0]), name=name, legendgroup=i+1), row=i+1, col=1)
        fig.add_trace(go.Scatter(x=[t[-1]], y=np.array(Rf[i]), line=dict(color=colors[2]), showlegend=False), row=i+1, col=1)
        fig.update_yaxes(title_text=f'x {i+1}', row=i+1, col=1)
        fig.update_xaxes(range=[0, t[-1]], row=i+1, col=1)
    for i in range(nu):
        fig.add_trace(go.Scatter(x=t, y=U[:,i], mode='lines', line=dict(color=colors[1]), name=name, legendgroup=nx+i+1), row=nx+i+1, col=1)
        fig.update_yaxes(title_text=f'u {i+1}', row=nx+i+1, col=1)
